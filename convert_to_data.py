...
with open('./data/DOE/data-2009-2010.csv') as csvfile:
    ...
    pk = 1
    for row in reader:
        high_enough_level_offered = (int(row['HLOFFER']) >= 4)
        more_than_post_secondary_offered = (int(row['DEGGRANT']) in [1, -3])
        if all([high_enough_level_offered, more_than_post_secondary_offered]):
            school = {
                'pk': pk,
                'model': 'judges.School',
                'fields': {
